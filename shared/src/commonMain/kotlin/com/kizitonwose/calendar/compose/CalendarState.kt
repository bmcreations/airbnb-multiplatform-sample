    ...
    companion object {
        internal val Saver: Saver<CalendarState, Any> = listSaver(
            save = {
                ...
                listOf(
                    Json.encodeToString(YearMonth.serializer(), it.startMonth),
                    Json.encodeToString(YearMonth.serializer(), it.endMonth),
                    Json.encodeToString(YearMonth.serializer(), it.firstVisibleMonth.yearMonth),
                    it.firstDayOfWeek,
                    it.outDateStyle,
                    Json.encodeToString(VisibleItemState.serializer(), visibleItemState),
                )
            },
            restore = {
                CalendarState(
                    startMonth = (it[0] as String).let { p -> Json.decodeFromString(p) },
                    endMonth = (it[1] as String).let { p -> Json.decodeFromString(p) },
                    firstVisibleMonth = (it[2] as String).let { p -> Json.decodeFromString(p) },
                    firstDayOfWeek = it[3] as DayOfWeek,
                    outDateStyle = it[4] as OutDateStyle,
                    visibleItemState = (it[5] as String).let { s -> Json.decodeFromString(s) },
                )
            },
        )
    }
